...
describe('which rows offer Resume', () => {
	it('offers it on a run whose image failed', () => {
		const markup = render(groupRows([textRow(), imageRow({ status: 'failed', costUsd: null, imageR2Key: null })]));
	...
	it('does not offer it on a run that already succeeded', () => {
		...
		expect(render(groupRows([textRow(), imageRow()]))).not.toContain('>Resume<');
	});

	it('does not offer it when the planner never succeeded', () => {
		const markup = render(groupRows([textRow({ status: 'failed', costUsd: null, plannerParams: {} })]));
...
describe('what a row shows', () => {
	it('renders an absent image row as absent, not as a blank cell', () => {
		const markup = render(groupRows([textRow({ status: 'failed', costUsd: null })]));
	...
	it('adds both rows up for the total', () => {
		const markup = render(groupRows([textRow({ costUsd: 0.001 }), imageRow({ costUsd: 0.0019008 })]));
