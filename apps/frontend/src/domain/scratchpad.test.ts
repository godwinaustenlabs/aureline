...
import type { RunResult } from './outcome';
...
describe('buildScratchpad', () => {
	it('separates the planner cost, the image cost and the real total', () => {
		...
		const cost = entries(buildScratchpad({ engine: 'helios', runId: null, result: null, group: group!, wallClockMs: null }), 'Cost');
	...
	it('labels the response figure as the image cost, never as the total', () => {
		const sections = buildScratchpad({ engine: 'helios', runId: null, result: { ...failedResult, cost_usd: 0.0019008 }, group: null, wallClockMs: 1200 });
	...
	it('names the stage a failure happened in', () => {
		const outcome = entries(buildScratchpad({ engine: 'helios', runId: null, result: failedResult, group: null, wallClockMs: 900 }), 'Outcome');
	...
	it('says a completed run did not fail rather than leaving a blank', () => {
		const outcome = entries(
			buildScratchpad({ engine: 'helios', runId: null, result: { ...failedResult, status: 'completed', error: null }, group: null, wallClockMs: 900 }),
			'Outcome',
		);
	...
	it('renders every "not captured" row, always', () => {
		const sections = buildScratchpad({ engine: 'helios', runId: null, result: null, group: null, wallClockMs: null });
	...
	it('labels the gaps rather than leaving them empty when there are no rows', () => {
		const planner = entries(buildScratchpad({ engine: 'helios', runId: null, result: failedResult, group: null, wallClockMs: null }), 'Planner — text row');
	...
	it('prefers the stored params over the response copy, and falls through an empty stored object', () => {
		const [withParams] = groupRows('helios', [heliosTextRow({ plannerParams: { motif_type: 'thistle' } })]);
		expect(entries(buildScratchpad({ engine: 'helios', runId: null, result: failedResult, group: withParams!, wallClockMs: null }), 'Params the planner produced')
			.get('plannerParams (stored)')?.json).toEqual({ motif_type: 'thistle' });

		// `{}` is what a text row holds before the planner settles it.
		const [empty] = groupRows('helios', [heliosTextRow({ plannerParams: {}, status: 'failed' })]);
		expect(entries(buildScratchpad({ engine: 'helios', runId: null, result: failedResult, group: empty!, wallClockMs: null }), 'Params the planner produced')
	...
	it('says a run has no lineage rather than showing three empty markers', () => {
		const [original] = groupRows('helios', [heliosTextRow(), heliosImageRow()]);
		const lineage = entries(buildScratchpad({ engine: 'helios', runId: null, result: null, group: original!, wallClockMs: null }), 'Resume lineage');

		expect(lineage.get('Lineage')?.missing).toBe(true);
	});

	it('shows the markers when the run is part of a resume chain', () => {
		const [resumed] = groupRows('helios', [
			heliosTextRow({ modelMetadata: { model: '@cf/x', root: 'r1', resumed_from: 'p1', attempt: 2, planner_skipped: true } }),
			heliosImageRow({ modelMetadata: { model: '@cf/flux', steps: 8, root: 'r1', resumed_from: 'p1', attempt: 2 } }),
		]);

		const lineage = entries(buildScratchpad({ engine: 'helios', runId: null, result: null, group: resumed!, wallClockMs: null }), 'Resume lineage');
