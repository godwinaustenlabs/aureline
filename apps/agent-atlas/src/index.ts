...
import { readGarmentImage } from "./repository/r2.repository";
import { preflight, withCors } from "./cors";

// The Durable Object class must be exported from the Worker's main module for
// wrangler's `class_name: "AtlasAgent"` binding to resolve. Delete this line and
// the binding breaks at deploy time, not at typecheck time.
export { AtlasAgent } from "./agent";
...
async function route(request: Request, env: Env): Promise<Response> {
	...
	if (url.pathname === "/") {
		return new Response("Atlas Agent is running", { status: 200 });
	}
	...
	if (url.pathname === "/generate" || url.pathname === "/resume" || url.pathname === "/runs") {
		...
		const agent = await getAgentByName(env.AtlasAgent, await scopeKey(request));
		return agent.fetch(request);
	}

	if (url.pathname.startsWith("/images/")) {
		// Everything after "/images/" is the R2 key, e.g. "atlas/{pipeline_id}.jpg"
		const key = url.pathname.slice("/images/".length);
		const object = await readGarmentImage(env.PATTERNS, key);
