 ...
 * (namespace "ATLAS_CONFIG"); this app only ever reads it. There is one
 * namespace, shared by local dev and production — see
 * docs/adr/0008-runtime-config-resolved-from-kv.md.
 *
 * The binding is named `CONFIG` and not `ATLAS_CONFIG` on purpose: every
 * engine's config.ts is then identical code, and the engines are told apart
 * only by the namespace's dashboard title.
 *
 * **Four keys, not Helios's and Iris's five.** There is no `text_model`,
 * because Atlas has no text call (ADR-ATLAS-0001). Copying the key across for
 * symmetry would give us a key with no reader, which somebody will eventually
 * set to fix an output, and nothing will happen.
 */
...
export interface AtlasConfig {
	...
	/**
	 * Read so all four keys resolve through the same code path Helios and Iris
	 * use. **It governs nothing on Atlas's image path**: nothing in Atlas
	 * auto-retries, because the one call is expensive, one-shot, and very likely
	 * to fail the same way twice (ADR-0009, ADR-ATLAS-0001). A person retries by
	 * calling `POST /resume`.
	 */
	maxRetries: number;
	retentionLimit: number;
	/**
	 * How many times one brief may be resumed. Counts resumes only, so 3 means an
	 * original plus three retries. Atlas has exactly one billable call, so this
	 * is precisely the ceiling on what a single brief can cost.
	 */
	maxResumeAttempts: number;
	/** Per-field provenance, for the log line. */
	source: Record<"imageModel" | "maxRetries" | "retentionLimit" | "maxResumeAttempts", ConfigSource>;
}
...
/**
 * The model key holds a JSON object (`{ "model": ..., "steps": ... }`), but the
 * dashboard is hand-edited and a bare model id is the obvious thing to type.
 * Accept both: anything not starting with `{` is treated as the model id.
 *
 ...
 *
 * Adding a tunable is one entry. `resolveConfig` and `describeConfig` both just
 * walk this table.
 *
 ...
 *
 * `text_model` is deliberately absent too. Atlas has no text call.
 */
const FIELDS = [
	{
		key: "image_model",
	...
	},
	{
		...
		// brief may spend the image model, so a fat-fingered dashboard edit
...
/** Renders a model config as `id` or `id(steps=4)` for the log line. */
function describeModel(value: ImageModelConfig): string {
 ...
 * cache.
 */
export async function resolveConfig(env: Env): Promise<AtlasConfig> {
	...
	return { ...resolved, source } as AtlasConfig;
}

/** One-line summary of the resolved config and where each value came from. */
export function describeConfig(config: AtlasConfig): string {
