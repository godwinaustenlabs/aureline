...
export async function completeTextRun(
	...
	await db
		...
		.where(and(eq(irisRuns.pInvocId, pInvocId), eq(irisRuns.modality, "text")));
}
...
export async function failRunningRuns(
	...
	await db
		...
		.where(and(eq(irisRuns.pInvocId, pInvocId), eq(irisRuns.status, "running")));
}
...
export async function getRunRows(db: IrisDb, pInvocId: string): Promise<IrisRun[]> {
