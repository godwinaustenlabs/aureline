...
describe("prompts repository", () => {
	...
	it("stores a prompt and reads it back", async () => {
		await upsertPrompt(d1, { slot: "iris_planner", promptText: PROMPT });

		const row = await getPrompt(d1, "iris_planner");

		expect(row?.slot).toBe("iris_planner");
		expect(row?.promptText).toBe(PROMPT);
	});

	it("returns null for a slot that has never been written", async () => {
		expect(await getPrompt(d1, "iris_planner")).toBeNull();
	});

	it("lists every stored prompt", async () => {
		await upsertPrompt(d1, { slot: "iris_planner", promptText: PROMPT });
		await upsertPrompt(d1, { slot: "iris_color", promptText: `${PROMPT} Second slot.` });

		const slots = (await listPrompts(d1)).map((row) => row.slot).sort();

		expect(slots).toEqual(["iris_color", "iris_planner"].sort());
	});
	...
	it("overwrites the existing row rather than adding a second one", async () => {
		await upsertPrompt(d1, { slot: "iris_planner", promptText: PROMPT });
		await upsertPrompt(d1, { slot: "iris_planner", promptText: "Rewritten." });

		const rows = await d1.select().from(prompts).where(eq(prompts.slot, "iris_planner"));
	...
	it("moves updated_at forward when a prompt is edited", async () => {
		...
		expect((await getPrompt(d1, "iris_planner"))?.updatedAt).not.toBe("2000-01-01 00:00:00");
	});
	...
	it("proves the column default does not fire on a plain UPDATE", async () => {
		...
		await d1.update(prompts).set({ promptText: "Rewritten." }).where(eq(prompts.slot, "iris_planner"));

		expect((await getPrompt(d1, "iris_planner"))?.updatedAt).toBe("2000-01-01 00:00:00");
	});

	it("refuses a blank prompt and stores nothing", async () => {
		await expect(upsertPrompt(d1, { slot: "iris_planner", promptText: "   " })).rejects.toThrow(/empty prompt/);
	...
	it("leaves an existing prompt intact when a blank edit is refused", async () => {
		...
		expect((await getPrompt(d1, "iris_planner"))?.promptText).toBe(PROMPT);
	});
	...
	it("rejects a second row for the same slot inserted around the repository", async () => {
		...
		await expect(
			d1.insert(prompts).values({ slot: "iris_planner", promptText: "Smuggled in." }),
		).rejects.toThrow();
	});
});
