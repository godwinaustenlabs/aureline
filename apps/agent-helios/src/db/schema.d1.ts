...
/**
 * Tables that live in Helios's **D1 database only**, never in the Durable
 * Object's own SQLite.
 *
 ...
 *
 * Deliberately identical to Iris's `schema.d1.ts` down to the column order.
 * The two engines keep separate D1 databases for the sprint, so this is two
 * tables rather than one, and keeping them the same shape is what lets the
 * playground read both through one piece of code and lets the rows merge into
 * a single table when the databases are consolidated.
 */
export const prompts = sqliteTable("prompts", {
   ...
   * though this table already sits in Helios's own database: it costs nothing
