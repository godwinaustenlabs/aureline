 ...
 *
 * The table lives in D1 alone, so every function here takes a `HeliosD1Db` and
 ...
 */
export async function listPrompts(d1: HeliosD1Db): Promise<Prompt[]> {
	return d1.select().from(prompts);
}

/** One slot's row, or `null` when nothing has been stored for it yet. */
export async function getPrompt(d1: HeliosD1Db, slot: PromptSlot): Promise<Prompt | null> {
