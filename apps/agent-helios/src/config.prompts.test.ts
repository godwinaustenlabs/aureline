...
const FALLBACK = "You are a textile pattern designer. Return only the JSON object.";
const STORED = "You are a textile pattern designer, rewritten in the playground.";
...
describe("resolvePrompt", () => {
	...
	it("sends the stored prompt when the row exists", async () => {
		await upsertPrompt(d1, { slot: "helios_planner", promptText: STORED });

		const resolved = await resolvePrompt(d1, "helios_planner", FALLBACK);
	...
	it("falls back and warns when the stored prompt is too short to be real", async () => {
		await d1.insert(prompts).values({ slot: "helios_planner", promptText: "todo" });

		const resolved = await resolvePrompt(d1, "helios_planner", FALLBACK);

		expect(resolved.text).toBe(FALLBACK);
		expect(resolved.source).toBe("code");
		expect(warn).toHaveBeenCalledWith(expect.stringContaining("helios_planner"));
	});

	it("treats a whitespace-only prompt as unusable", async () => {
		await d1.insert(prompts).values({ slot: "helios_planner", promptText: "                              " });
	...
	it("falls back and warns when the database read throws", async () => {
		...
		const resolved = await resolvePrompt(failing, "helios_planner", FALLBACK);
...
describe("describePrompt", () => {
	it("names the store and the length, and the edit time when there is one", () => {
		expect(describePrompt("helios_planner", { text: STORED, source: "db", updatedAt: "2026-09-01 12:45:16" })).toBe(
			`prompt: helios_planner=${STORED.length}chars (db, updated 2026-09-01 12:45:16)`,
		);
	});

	it("omits the edit time for the committed prompt, which has none", () => {
		expect(describePrompt("helios_planner", { text: FALLBACK, source: "code", updatedAt: null })).toBe(
			`prompt: helios_planner=${FALLBACK.length}chars (code)`,
		);
	});
});
