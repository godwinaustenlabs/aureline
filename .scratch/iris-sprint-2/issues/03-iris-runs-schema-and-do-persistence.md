...
9. **`width` and `height` live inside `model_metadata`, not as columns of their own.** `IrisResultSchema` carries both, because Atlas needs to know what it is placing without fetching and decoding the image first. The table does not, and that is deliberate: thirteen columns is what makes iris-11's seven-row insert chunk fit under D1's 100-parameter cap, and two more columns would force it down to six. So the image row's `model_metadata` **must** carry `width` and `height` on every settled image row, and whatever reconstructs an `IrisResult` from stored rows reads them from there. This is a real obligation, not a convention: if the image stage omits them, the dimensions survive only in the live HTTP response and are gone for good once the run is exported and pruned, which defeats the whole reason the fields exist. iris-05 writes them from the fixture and iris-09 writes the real ones.
